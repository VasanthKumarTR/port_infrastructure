...
# Global configuration array - using quoted keys to prevent octal interpretation
declare -A CONFIG
...
show_config_status() {
    ...
    # Port.io
    if [[ -n "${CONFIG[port_client_id]}" && -n "${CONFIG[port_client_secret]}" ]]; then
        echo -e "  ${GREEN}✅ Port.io Credentials${NC} - Client ID: ${CONFIG[port_client_id]:0:8}..."
    else
        echo -e "  ${RED}❌ Port.io Credentials${NC} - Not configured"
    fi
    
    # Environment
    if [[ -n "${CONFIG[environment]}" && -n "${CONFIG[team_email]}" ]]; then
        echo -e "  ${GREEN}✅ Environment Settings${NC} - ${CONFIG[environment]} (${CONFIG[team_email]})"
    else
        echo -e "  ${RED}❌ Environment Settings${NC} - Not configured"
    fi
    
    # AWS
    if [[ -n "${CONFIG[aws_access_key_id]}" ]]; then
        echo -e "  ${GREEN}✅ AWS Integration${NC} - Region: ${CONFIG[aws_region]:-us-west-2}"
    else
        echo -e "  ${YELLOW}○ AWS Integration${NC} - Optional (not configured)"
    fi
    
    # GitHub
    if [[ -n "${CONFIG[github_app_id]}" ]]; then
        echo -e "  ${GREEN}✅ GitHub Integration${NC} - App ID: ${CONFIG[github_app_id]}"
    else
        echo -e "  ${YELLOW}○ GitHub Integration${NC} - Optional (not configured)"
    fi
    
    # Azure
    if [[ -n "${CONFIG[azure_client_id]}" ]]; then
        echo -e "  ${GREEN}✅ Azure Integration${NC} - Tenant: ${CONFIG[azure_tenant_id]:0:8}..."
    else
        echo -e "  ${YELLOW}○ Azure Integration${NC} - Optional (not configured)"
    fi
    
    # Azure DevOps
    if [[ -n "${CONFIG[azdo_organization_url]}" ]]; then
        echo -e "  ${GREEN}✅ Azure DevOps${NC} - ${CONFIG[azdo_organization_url]}"
    else
        echo -e "  ${YELLOW}○ Azure DevOps${NC} - Optional (not configured)"
    fi
    
    # Snyk
    if [[ -n "${CONFIG[snyk_token]}" ]]; then
        echo -e "  ${GREEN}✅ Snyk Integration${NC} - Org: ${CONFIG[snyk_organization]}"
    else
...
configure_port_credentials() {
    ...
    if [[ -n "${CONFIG[port_client_id]}" ]]; then
        echo -e "${BLUE}Current Client ID:${NC} ${CONFIG[port_client_id]}"
        echo ""
    ...
    while true; do
        ...
        if command -v curl &> /dev/null && command -v jq &> /dev/null; then
            if test_port_credentials "$port_client_id" "$port_client_secret"; then
                CONFIG["port_client_id"]="$port_client_id"
                CONFIG["port_client_secret"]="$port_client_secret"
                CONFIG["port_base_url"]="https://api.getport.io"
...
configure_environment() {
    ...
    # Show existing if available
    if [[ -n "${CONFIG[environment]}" && -n "${CONFIG[team_email]}" ]]; then
        echo -e "${BLUE}Current Environment:${NC} ${CONFIG[environment]}"
        echo -e "${BLUE}Current Team Email:${NC} ${CONFIG[team_email]}"
        echo ""
    ...
    while true; do
        ...
        case $env_choice in
            1) CONFIG["environment"]="dev"; break;;
            2) CONFIG["environment"]="staging"; break;;
            3) CONFIG["environment"]="prod"; break;;
    ...
    while true; do
        ...
        if validate_email "$team_email"; then
            CONFIG["team_email"]="$team_email"
            break
        else
...
configure_aws() {
    ...
    # Show existing if available
    if [[ -n "${CONFIG[aws_access_key_id]}" ]]; then
        echo -e "${BLUE}Current AWS Access Key:${NC} ${CONFIG[aws_access_key_id]:0:8}..."
        echo -e "${BLUE}Current AWS Region:${NC} ${CONFIG[aws_region]}"
        echo ""
...
configure_github() {
    ...
    # Show existing if available
    if [[ -n "${CONFIG[github_app_id]}" ]]; then
        echo -e "${BLUE}Current GitHub App ID:${NC} ${CONFIG[github_app_id]}"
        echo -e "${BLUE}Current GitHub Organization:${NC} ${CONFIG[github_organization]}"
        echo ""
    ...
    if [[ $configure_github =~ ^[Yy]$ ]]; then
        ...
        if [[ -n "$github_app_id" && -n "$github_installation_id" && -n "$github_organization" && -n "$github_private_key" ]]; then
            CONFIG["github_app_id"]="$github_app_id"
            CONFIG["github_installation_id"]="$github_installation_id"
            CONFIG["github_organization"]="$github_organization"
            CONFIG["github_actions_repo"]="$github_actions_repo"
            CONFIG["github_private_key"]="$github_private_key"
            CONFIG["github_actions_webhook_url"]="https://api.github.com/repos/$github_organization/$github_actions_repo/dispatches"
        ...
        unset CONFIG["github_app_id"]
        unset CONFIG["github_installation_id"]
        unset CONFIG["github_organization"]
        unset CONFIG["github_actions_repo"]
        unset CONFIG["github_private_key"]
        unset CONFIG["github_actions_webhook_url"]
    fi
}
...
configure_azure() {
    ...
    # Show existing if available
    if [[ -n "${CONFIG[azure_client_id]}" ]]; then
        echo -e "${BLUE}Current Azure Client ID:${NC} ${CONFIG[azure_client_id]}"
        echo -e "${BLUE}Current Azure Tenant ID:${NC} ${CONFIG[azure_tenant_id]}"
        echo ""
    ...
    read -p "Do you want to configure Azure integration? (y/n): " configure_azure
    ...
    if [[ $configure_azure =~ ^[Yy]$ ]]; then
        ...
        unset CONFIG["azure_client_id"]
        unset CONFIG["azure_client_secret"]
        unset CONFIG["azure_tenant_id"]
        unset CONFIG["azure_subscription_id"]
    fi
}
...
configure_azure_devops() {
    ...
    # Show existing if available
    if [[ -n "${CONFIG[azdo_organization_url]}" ]]; then
        echo -e "${BLUE}Current Azure DevOps Organization:${NC} ${CONFIG[azdo_organization_url]}"
        echo ""
    ...
    if [[ $configure_azdo =~ ^[Yy]$ ]]; then
        echo ""
        read -p "Enter Azure DevOps Organization URL: " azdo_organization_url
        read -s -p "Enter Azure DevOps Personal Access Token: " azdo_personal_token
        echo ""
        
        if [[ -n "$azdo_organization_url" && -n "$azdo_personal_token" ]]; then
            CONFIG["azdo_organization_url"]="$azdo_organization_url"
            CONFIG["azdo_personal_token"]="$azdo_personal_token"
        ...
        unset CONFIG["azdo_organization_url"]
        unset CONFIG["azdo_personal_token"]
    fi
}
...
configure_snyk() {
    ...
    # Show existing if available
    if [[ -n "${CONFIG[snyk_token]}" ]]; then
        echo -e "${BLUE}Current Snyk Organization:${NC} ${CONFIG[snyk_organization]}"
        echo ""
    ...
    read -p "Do you want to configure Snyk integration? (y/n): " configure_snyk
    
    if [[ $configure_snyk =~ ^[Yy]$ ]]; then
        echo ""
        read -s -p "Enter Snyk API Token: " snyk_token
        echo ""
        read -p "Enter Snyk Organization ID: " snyk_organization
        
        if [[ -n "$snyk_token" && -n "$snyk_organization" ]]; then
            CONFIG["snyk_token"]="$snyk_token"
            CONFIG["snyk_organization"]="$snyk_organization"
        ...
        unset CONFIG["snyk_token"]
        unset CONFIG["snyk_organization"]
    fi
}
...
save_configuration() {
    ...
    if [[ -z "${CONFIG[port_client_id]}" || -z "${CONFIG[port_client_secret]}" ]]; then
        print_error "Port.io credentials are required before saving!"
        return 1
    fi
    
    if [[ -z "${CONFIG[environment]}" || -z "${CONFIG[team_email]}" ]]; then
...
port_base_url      = "${CONFIG[port_base_url]}"
...
github_actions_webhook_url = "${CONFIG[github_actions_webhook_url]}"
...
azdo_organization_url = "${CONFIG[azdo_organization_url]}"
azdo_personal_token   = "${CONFIG[azdo_personal_token]}"
...
snyk_organization = "${CONFIG[snyk_organization]}"
...
github_actions_repo = "${CONFIG[github_actions_repo]:-}"
